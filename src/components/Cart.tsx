          ...
          >
            ...
            {cartItems.length > 0 ? (
                  ...
                  <ul className="space-y-6">
                    <AnimatePresence>
                      {cartItems.map((item: CartItem) => (
                        ...
                        >
                          <Image src={item.image_url || '/placeholder.png'} alt={item.name} width={80} height={80} className="rounded-lg border border-gray-200 object-cover"/>
                          <div className="flex-grow">
                            <p className="font-semibold text-gray-800 text-sm">{item.name}</p>
                            <p className="text-xs text-gray-600 mt-1">{new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(item.price)}</p>
                          </div>
                          <div className="flex items-center border border-gray-300 rounded-md">
                            <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="px-3 py-1 text-gray-700 hover:bg-gray-100 transition-colors">-</button>
                            {/* PERBAIKAN DI SINI */}
                            <span className="px-3 text-sm font-semibold text-gray-900">{item.quantity}</span>
                            <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="px-3 py-1 text-gray-700 hover:bg-gray-100 transition-colors">+</button>
                          </div>
                          <button onClick={() => removeFromCart(item.id)} title="Hapus item" className="text-gray-400 hover:text-red-500 transition-colors p-1 rounded-full">
              ...
              <div className="flex-grow flex flex-col items-center justify-center text-center p-6">
                <EmptyCartIcon />
                <h3 className="text-lg font-semibold text-gray-700 mt-6">Keranjang Anda kosong</h3>
                <p className="text-sm text-gray-500 mt-2">Ayo cari produk terbaik untukmu!</p>
                <Link href="/" onClick={onClose} className="mt-8 bg-gray-800 text-white px-6 py-2.5 rounded-lg hover:bg-gray-900 transition-transform transform hover:scale-105 shadow-md">
                  Mulai Belanja
                </Link>
              </div>
            )}
