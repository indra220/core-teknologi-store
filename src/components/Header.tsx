      ...
      <header className={`sticky top-0 z-50 transition-all duration-300 ${headerClasses}`}>
        <nav className="container mx-auto flex justify-between items-center p-4">
          ...
          <div className="flex items-center space-x-2 sm:space-x-4 text-gray-800 dark:text-gray-200">
            {user && profile ? (
              <>
                <button onClick={() => setIsCartOpen(true)} className="relative p-2 hover:bg-black/5 rounded-full dark:hover:bg-white/10">
                  <CartIcon />
                  {cartCount > 0 && (
                ...
                <div className="relative">
                  <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="text-sm hidden sm:block hover:text-gray-600 dark:hover:text-gray-50">
                    Halo, {profile.displayName}
                  </button>
                  <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="p-2 block sm:hidden hover:bg-black/5 rounded-full dark:hover:bg-white/10">
                    <UserIcon />
                  </button>
                  {isDropdownOpen && (
              ...
              <>
                ...
                <Link href="/login" className="hover:text-gray-600 dark:hover:text-gray-50 text-sm sm:text-base">Login</Link>
                <Link href="/register" className="bg-blue-600 px-3 sm:px-4 py-2 rounded-lg text-white hover:bg-blue-700 text-sm">Register</Link>
              </>
            )}
          </div>
        </nav>
